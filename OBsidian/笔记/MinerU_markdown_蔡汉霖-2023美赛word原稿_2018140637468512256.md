...
作为智能手机中最直观的耗电组件，屏幕的功耗特性由其底层显示技术决定。本文以当前旗舰手机广泛采用的 OLED技术为研究对象，将屏幕瞬时功率建模为以下线性方程：$$P_{disp}(t) = P_{driver} + \sigma \cdot A_{area} \cdot B(t) \cdot \eta(t)$$
**参数定义：**
...
处理器负责逻辑运算与多任务调度。基于 CMOS 数字集成电路的物理特性，动态功耗与时钟频率及操作电压的平方成正比。考虑到现代芯片的动态电压频率调整机制，本文将其功耗抽象为关于频率和负载率的连续函数。

$$P_{cpu}(t) = \underbrace{P_{idle}}_{\text{Static leakage}} + \underbrace{\kappa_{cpu} \cdot f(t) \cdot u(t)}_{\text{Dynamic power}}$$
...
网络连接模块是智能手机中一个重要的耗电单元。其功耗动态受**自适应发射功率控制**机制主导。根据无线通信中的 **Friis** **传输方程**，信号在传播过程中会经历路径损耗。网络连接模块为了维持与基站的有效连接，手机必须根据接收到的信号强度指示反向调整发射功率。故本文基于 **Friis 传输方程** 与射频功耗特性，建立网络模组的瞬时功率模型如下：
...
- $S(t)$：接收信号强度 (Signal strength, dBm)。注意 $S(t)$ 为负值。
