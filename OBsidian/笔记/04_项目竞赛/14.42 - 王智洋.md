...
## 5 .2 仿真场景验证与误差分析
### 5.2.1 仿真场景建模与参数设定
选取 iPhone 15 Pro 作为基准对象，构建三种典型负载场景（表 5-1），分别对应高功率游戏（S1）、流媒体播放（S2）及低功耗阅读（S3）。
...
如图 15所示，仿真结果呈现出显著的升温延寿效应：散热受限组在放电末端并未性能衰减，反而较标准工况延长了约 **23 秒** 的续航时间。这一现象的物理本质在于，随着 $SOC$ 耗尽，维持恒功率所需的电流激增导致在弱散热条件下，电池内部温度快速升高，电化学内阻迅速降低。内阻的降低减缓了由 $IR$ 压降引发的电压崩塌速率，推迟了截止电压的触发。该结果有力验证了本模型在处理高功率放电末期复杂的电-热耦合动态时，具备极高的物理保真度与预测可靠性。
...
利用标定后的模型以及真实原始数据，我们进行了分析，结果如图 16所示。
...
## 8.4模型的局限 
1.本文采用了集总参数热模型，假设电池单体内部温度分布均匀。然而，在实际物理过程中，不同元器件区域之间存在显著的热梯度（Thermal Gradient）。这种空间上的均一化假设可能导致对局部热点（Hotspots）温度的低估。若将该模型迁移至电动汽车的大型电池模组，必须引入空间分布热模型（如有限元分析）以修正局部过热风险。 2. OCV 磁滞效应的忽略 (Neglect of OCV Hysteresis)在 3.2.2 节的 OCV-SOC 建模中，我们采用了单一的平均电压曲线，忽略了电池充放电路径不重合的电压磁滞现象。影响：在电池频繁进行“充-放”切换的浅循环工况下，磁滞效应可能导致 SOC 初始估计产生 $1\% \sim 2\%$ 的偏差。未来的工作可改进引入 磁滞模型，通过记录历史电流方向来进一步收窄开路电压的预测误差。